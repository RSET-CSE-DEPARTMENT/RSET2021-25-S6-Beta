...
def parse_text(text):
    ...
    if language:
        ...
        write_entities_pos_to_csv(original_malayalam_text, translated_entities, './static/new_entities.csv')
        # Write POS tags to CSV file
        write_entities_pos_to_csv(original_malayalam_text, filtered_translated_tokens, './static/new_pos_tags.csv')
        # Write sentiment and sentence to CSV file
        with open('./static/new_sentiment.csv', 'a', newline='', encoding='utf-8') as file:
            writer = csv.writer(file)
...
open_and_parse_csv('./static/all_words.csv')
